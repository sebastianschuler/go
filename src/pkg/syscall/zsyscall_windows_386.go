...
var (
	...
	procWSAStartup = modws2.NewProc("WSAStartup")
	procWSACleanup = modws2.NewProc("WSACleanup")
	procWSAIoctl = modws2.NewProc("WSAIoctl")
	procsocket = modws2.NewProc("socket")
	procsetsockopt = modws2.NewProc("setsockopt")
	procgetsockopt = modws2.NewProc("getsockopt")
	procbind = modws2.NewProc("bind")
	procconnect = modws2.NewProc("connect")
	procgetsockname = modws2.NewProc("getsockname")
	procgetpeername = modws2.NewProc("getpeername")
	proclisten = modws2.NewProc("listen")
	procshutdown = modws2.NewProc("shutdown")
	procclosesocket = modws2.NewProc("closesocket")
	procAcceptEx = modmswsock.NewProc("AcceptEx")
	procGetAcceptExSockaddrs = modmswsock.NewProc("GetAcceptExSockaddrs")
	procWSARecv = modws2.NewProc("WSARecv")
	procWSASend = modws2.NewProc("WSASend")
	procWSARecvFrom = modws2.NewProc("WSARecvFrom")
	procWSASendTo = modws2.NewProc("WSASendTo")
	procgethostbyname = modws2.NewProc("gethostbyname")
	procgetservbyname = modws2.NewProc("getservbyname")
	procntohs = modws2.NewProc("ntohs")
	procgetprotobyname = modws2.NewProc("getprotobyname")
	procAFPDnsQuery_W = modcoredll.NewProc("AFPDnsQuery_W")
	procAfdDNSRecordListFree = modcoredll.NewProc("AfdDNSRecordListFree")
	procGetAddrInfoW = modws2.NewProc("GetAddrInfoW")
	procFreeAddrInfoW = modws2.NewProc("FreeAddrInfoW")
	procGetIfEntry = modiphlpapi.NewProc("GetIfEntry")
	procGetAdaptersInfo = modiphlpapi.NewProc("GetAdaptersInfo")
	procSetFileCompletionNotificationModes = modcoredll.NewProc("SetFileCompletionNotificationModes")
	procWSAEnumProtocolsW = modws2.NewProc("WSAEnumProtocolsW")
...
func DnsQuery(name string, qtype uint16, options uint32, extra *byte, qrs **DNSRecord, pr *byte) (status error) {
	...
	r0, _, _ := Syscall6(procAFPDnsQuery_W.Addr(), 6, uintptr(unsafe.Pointer(_p0)), uintptr(qtype), uintptr(options), uintptr(unsafe.Pointer(extra)), uintptr(unsafe.Pointer(qrs)), uintptr(unsafe.Pointer(pr)))
	if r0 != 0 {
		status = Errno(r0)
	}
	return
...
func DnsRecordListFree(rl *DNSRecord, freetype uint32) {
	Syscall(procAfdDNSRecordListFree.Addr(), 2, uintptr(unsafe.Pointer(rl)), uintptr(freetype), 0)
	return
}
